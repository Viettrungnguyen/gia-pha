 ...
 * @file src/lib/dev-fake-compact-tree.ts
 * @description Dev-only compact fake tree (5 generations, multi-wife, sort_order mix).
 *              Designed to test:
 *                - CV1: sort_order hiển thị đúng (NULLS LAST), fallback birth_year
 *                - CV2: cây compact: 1 ô couple + nhiều vợ, con gái gộp ô
 *
 *              Cấu trúc 5 đời:
 *                Đời 1: Ông Tổ A + Tổ Mẫu B
 *                Đời 2: 3 con trai (C1, C2, C3) - 1 thử multi-wife
 *                Đời 3: mỗi ông 2 con gái + 1 con trai
 *                Đời 4: 4 cháu nội (đa chi)
 *                Đời 5: 5 chắt
 *
 *              Bật bằng NEXT_PUBLIC_USE_FAKE_TREE=compact.
 * @version 2.0.0
 * @updated 2026-09-05
 */
...
/**
 * Bảng 30+ người qua 5 đời (không có người "chưa rõ").
 */
function build(): TreeData {
  const rawPeople: Record<string, RawPerson> = {
    ...
    // ===== Đời 2: 3 con trai + 3 vợ (ông C lấy 2 vợ) =====
    ...
    // ===== Đời 3: 9 người (3 ông × 3 con, gồm cả gái) =====
    // Ông C1 + D1: 3 con (E1, E2, E3)
    ...
    // Ông C3 + D3: 2 con gái (test ô con gái nhiều)
    ...
    // Vợ của mấy ông đời 3 (để có thể lập gia đình ở đời 4)
  ...
  const rawFamilies: RawFamily[] = [
    // ===== Đời 1 -> Đời 2 (Ông Tổ A + Bà Tổ Mẫu B) =====
    ...
    // ===== Đời 2 -> Đời 3: Ông C1 lấy 2 vợ (D1, H1) =====
    // Family 1: C1 + D1
    { key: 'F2', husband_key: 'C1', wife_key: 'D1', child_keys: [
      { key: 'E1', sort_order: 0 },
      { key: 'E2', sort_order: 1 },
      { key: 'E3' /* sort undefined → NULLS LAST */ },
    ] },
    // Family 2: C1 + H1 (vợ 2)
    ...
    // C3 + D3 - 3 con gái (test nhiều con gái trong 1 ô)
  ...
  const idCounter = { v: 0 };
  const nextId = () => `fake-c-${(++idCounter.v).toString().padStart(4, '0')}`;
  const keyToId = new Map<string, string>();

  const people: Person[] = Object.values(rawPeople).map((rp) => {
    const id = nextId();
    keyToId.set(rp.key, id);
    // Tự dò surname dựa vào giới tính và pattern "Nguyễn ...".
    // Chuẩn hoá: nam = "Nguyễn Đình", nữ = "Nguyễn Thị".
    const surname = rp.gender === 1 ? 'Nguyễn Đình' : 'Nguyễn Thị';
    // Tên (display_name) đã chứa cả họ. first_name = phần cuối cùng.
    const parts = rp.display_name.split(/\s+/);
    const firstName = parts[parts.length - 1] ?? null;
    return {
      id,
      handle: `compact-${rp.key.toLowerCase()}`,
...
export function generateCompactFakeTree(): TreeData {
  return build();
}
